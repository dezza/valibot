...
First you create a schema. A schema can be compared to a type definition in TypeScript. The big difference is that TypeScript types are "not executed" and are more or less a DX (*Developer experience*) feature. A schema on the other hand, apart from the inferred type definition, can also be executed at runtime to guarantee type safety of unknown data.

```ts
import { email, minLength, object, type Output, parse, string } from 'valibot'; // 0.87 kB
...
Apart from <Link href="/api/parse">`parse`</Link> I also offer a non-exception-based API with <Link href="/api/safeParse">`safeParse`</Link> and a type guard function with <Link href="/api/is">`is`</Link>. You can find more about it <Link href="/guides/parse-data">here</Link>.
